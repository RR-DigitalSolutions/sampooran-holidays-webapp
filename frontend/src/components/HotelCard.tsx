      ...
      <div className="group w-full bg-primary rounded-xl overflow-hidden border border-primary/20 hover:shadow-[0_20px_50px_rgba(0,0,0,0.3)] transition-all duration-500 flex flex-col h-full relative">
        ...
        <div className="relative h-28 xs:h-36 sm:h-56 overflow-hidden shrink-0 w-full">
          ...
          <div className="absolute top-2 left-2 sm:top-3 sm:left-3 flex gap-1 sm:gap-1.5 flex-wrap z-10">
            ...
            {hotel.starRating >= 4 && (
              <span className="bg-[#ff8f00] text-white text-[7px] font-medium px-1.5 py-0.5 sm:px-2.5 sm:py-1 rounded-full uppercase tracking-wider shadow-xs flex items-center gap-0.5 font-sans">
                Premium Stay
              </span>
            )}
            {hotel.isVerified && (
          ...
          <div className="absolute bottom-2 inset-x-2 sm:bottom-3 sm:inset-x-3.5 z-10 flex items-center justify-between gap-1.5">
            <div className="flex items-center gap-1 text-white/95 max-w-[70%]">
              <MapPin className="h-2.5 w-2.5 text-[#ff8f00] shrink-0" />
            ...
            <div className="flex items-center gap-0.5 bg-[#ff8f00] text-white px-1.5 py-0.5 sm:px-2 sm:py-0.5 rounded-lg shadow-sm border border-white/10 shrink-0">
              <Star className="h-2.5 w-2.5 fill-current" />
              <span className="text-[7px] sm:text-[8px] font-medium font-sans">{hotel.starRating || 3}.0</span>
            </div>
          </div>
        </div>
          ...
          <div className="flex gap-1.5 sm:gap-4 mb-3 sm:mb-5 mt-0.5 sm:mt-1 font-sans">
            {[
              ...
              <div key={i} className="flex items-center gap-1 sm:gap-1.5 text-white/70 group-hover:text-white transition-colors" title={item.label}>
                ...
                <span className="hidden min-[380px]:inline-block text-[7px] sm:text-[8px] font-medium uppercase tracking-wider">{item.label}</span>
              </div>
            ))}
          </div>

          {/* Footer Area */}
          <div className="mt-auto pt-2.5 sm:pt-4 border-t border-white/10 flex items-center justify-between w-full">
            <div className="font-sans">
              <p className="text-[7px] sm:text-[8px] text-white/50 uppercase font-black tracking-widest mb-0.5">Best Rate From</p>
              <div className="flex items-baseline gap-0.5">
                <span className="text-sm sm:text-xl font-bold text-white tracking-tighter font-sans">₹{(hotel.startingPrice || 2500).toLocaleString('en-IN')}</span>
                <span className="text-[7.5px] sm:text-[9px] font-black text-white/50 uppercase font-sans">
