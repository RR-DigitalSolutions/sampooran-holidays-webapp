      ...
      >
        <div className="group w-full bg-primary rounded-xl overflow-hidden border border-primary/20 hover:shadow-[0_20px_50px_rgba(0,0,0,0.3)] transition-all duration-500 flex flex-col h-full relative">
          ...
          <div className="relative h-48 sm:h-56 overflow-hidden shrink-0 w-full">
            ...
            <div className="absolute bottom-3 inset-x-3.5 z-10 flex items-center gap-2">
          ...
          <div className="p-4 flex flex-col flex-1 bg-primary text-white">
            ...
            <div className="flex gap-4 mb-5 mt-1 font-sans">
              {[
                { icon: Users, label: `${vehicle.capacity} Seats` },
                { icon: Luggage, label: "Cargo Space" },
                { icon: Gauge, label: "Aircon" }
              ].map((item, i) => (
            ...
            {/* Footer Area */}
            <div className="mt-auto pt-4 border-t border-white/10 flex items-center justify-between w-full font-sans">
               <div>
                  <p className="text-[8px] text-white/50 font-black uppercase tracking-widest mb-0.5">Starting Daily</p>
                  <div className="flex items-baseline gap-0.5">
                     <span className="text-lg md:text-xl font-bold text-white tracking-tighter font-sans">₹{(vehicle.pricePerDay || 2500).toLocaleString('en-IN')}</span>
                     <span className="text-[9px] font-black text-white/50 uppercase">/ Day</span>
                  </div>
               </div>
               <Link href="/transport">
                 <button
