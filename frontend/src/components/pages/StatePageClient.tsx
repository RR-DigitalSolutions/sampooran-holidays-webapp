...
export default function StatePageClient({ stateSlug: propStateSlug }: { stateSlug?: string }) {
  const params = useParams<{ state: string; country?: string }>();
  const stateSlug = propStateSlug || params?.state || params?.country;
  const [viewMode, setViewMode] = useState<"list" | "grid">("list");
  
  // Dynamic state data from API
  const { data: dbState } = useQuery({
    queryKey: ["state", stateSlug],
    queryFn: () => customFetch<any>(`/api/states/${stateSlug}`),
    enabled: !!stateSlug,
  });
  ...
  return (
    <div className="bg-slate-50 font-sans min-h-screen">
      ...
      <section className="relative h-[65vh] min-h-[480px] max-h-[720px] overflow-hidden group">
        <motion.div 
          initial={{ scale: 1.1 }}
          animate={{ scale: 1 }}
            ...
            <div className="bg-white rounded-[2rem] p-8 shadow-xl shadow-slate-200/40 border border-slate-100">
              ...
              <div className="space-y-2">
                {displayPlaces.map((city: any, idx: number) => (
                  <Link 
                    key={idx} 
                    href={city.id ? `/destinations/${city.slug}` : `/india/${stateSlug}/${city.slug}`}
            ...
            {displayPlaces.length > 0 && (
              <section id="places" className="scroll-mt-32 pt-10 border-t border-slate-200">
                 ...
                 <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
                    {displayPlaces.map((city: any, idx: number) => (
                      <Link key={idx} href={city.id ? `/destinations/${city.slug}` : `/india/${stateSlug}/${city.slug}`}>
                    ...
                    >
                      <AccordionTrigger className="hover:no-underline px-6 py-5 text-left text-slate-800 font-bold text-base data-[state=open]:text-primary [&>svg]:hidden">
                        <div className="flex items-center gap-4 w-full pr-4">
                          <span className="shrink-0 w-7 h-7 rounded-full bg-slate-100 text-slate-400 text-xs font-black flex items-center justify-center data-[state=open]:bg-primary data-[state=open]:text-white transition-all">
                            {i + 1}
                          </span>
